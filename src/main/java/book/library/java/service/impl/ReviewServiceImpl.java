...
public class ReviewServiceImpl extends AbstractServiceImpl<Review, ReviewPattern> implements ReviewService {
    ...
    @Override
    public Integer create(Review review) throws BusinessException { // todo: why DaoException in signature?
        validateReview(review);
        return super.create(review);
    }

    @Override
    public ListEntityPage<ReviewDto> readReviews(ListParams listParams) throws BusinessException { // todo: generic for ListParams!, why DaoException in signature?
	    ...
	    // todo: stream + map
        listEntity.forEach(review -> reviewDtoList.add(new ReviewDto(review.getId(), review.getCommenterName(), review.getComment(), review.getRating())));
