...
class BooksIndex {
    ...
    constructor(private booksApi: IBooksApi, private authorsApi: IAuthorsApi, private $uibModal: ng.ui.bootstrap.IModalService, private $routeParams: IRouteParams) {
        if (!isNaN(parseInt($routeParams.isbn))) {
            ...
            this.authorsApi.getById(this.authorId).then(author => {
                this.authorWithBooks = author.data
            });
        }
    ...
    pageChanged(page) {
        ...
        this.booksApi.find(new ListParams(this.limit, this.offset, new BookPattern(this.authorId, this.search, this.rating), this.sortParam))
            .then(booksAndCountPages => {
                this.booksAndCountPages = booksAndCountPages;
                this.booksAndCountPages.list.forEach(book => book.averageRatingRound = Math.round(book.averageRating))
            });
        this.checkAll = false;
    }
    ...
    check(bookId) {
        ...
        this.booksAndCountPages.list.forEach(book => {
            if (book.id === bookId) {
                ...
                if (book.removeStatus === false) {
                    this.checkAll = false
                }
            }
            if (book.removeStatus === true) {
    ...
    bulkDeleteBooks(booksRemove: IBook[], idEntities: number[]) {
        ...
        idEntities = [];
        this.booksAndCountPages.list.forEach(book => {
            if (book.removeStatus) {
                booksRemove.push(book);
                idEntities.push(book.id)
            }
        });
        this.dialog = this.$uibModal.open({
...
class AddBook {
    ...
    pageChanged() {
        ...
        this.booksApi.find(new ListParams(this.limit, this.offset, new BookPattern(this.authorId, this.search, null), null))
            .then(booksAndCountPages => {
                this.booksAndCountPages.list = booksAndCountPages.list;
                this.booksAndCountPages.totalItems = booksAndCountPages.totalItems;
                this.booksAndCountPages.list.forEach(book => book.averageRatingRound = Math.round(book.averageRating)
            });
    }
